...
const App = () => {
  ...
  const convertWeights = () => {
    ...
    if (total <= 0) return alert("Wpisz wagi mięsa większe niż 0!");
    const newRatios = {};
  ...
  const handleSave = async () => {
    if (!newRecipe.name) return alert("Podaj nazwę wyrobu!");
    if (sumPercent !== 100) return alert("Suma składników musi wynosić 100%!");
    ...
    try {
      ...
      alert("Receptura została zapisana pomyślnie!");
    } catch (e) { alert("Błąd połączenia z bazą!"); }
  };
          ...
          <div className="bg-white rounded-[2.5rem] p-6 md:p-10 shadow-2xl border-t-[12px] border-amber-400 no-print space-y-8 animate-in fade-in zoom-in duration-300">
            ...
            <div className="grid lg:grid-cols-2 gap-10">
              <div className="space-y-6">
                <input type="text" className="w-full border-2 border-slate-100 rounded-2xl p-5 text-xl font-black bg-slate-50 focus:bg-white focus:border-blue-500 outline-none transition-all" value={newRecipe.name} onChange={e => setNewRecipe({...newRecipe, name: e.target.value})} placeholder="Nazwa wyrobu..." />
                ...
                <div className="p-6 bg-slate-100 rounded-[2.5rem] border-2 border-slate-200 shadow-inner">
                  ...
                  <div className="flex bg-white p-2 rounded-2xl mb-8 border-2 border-blue-500 shadow-lg">
                    <button type="button" onClick={() => setCompMode('percent')} className={`flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${compMode === 'percent' ? 'bg-blue-600 text-white shadow-md' : 'text-blue-600'}`}>Ustaw Procenty %</button>
                    <button type="button" onClick={() => setCompMode('weight')} className={`flex-1 py-4 rounded-xl font-black text-xs uppercase tracking-widest transition-all ${compMode === 'weight' ? 'bg-blue-600 text-white shadow-md' : 'text-blue-600'}`}>Wpisz Wagi (kg)</button>
                  </div>
                  ...
                  <div className="flex justify-between items-center mb-8 bg-white p-4 rounded-2xl shadow-sm border border-slate-200">
                    <span className={`font-black text-xl ${compMode === 'percent' ? (sumPercent === 100 ? 'text-green-600' : 'text-amber-600') : 'text-blue-600'}`}>
                        {compMode === 'percent' ? `SUMA: ${sumPercent}%` : `SUMA WAG: ${sumKg} kg`}
                    </span>
                    {compMode === 'weight' ? (
                        ...
                        <div>
                          ...
                          {compMode === 'percent' ? (
                            ...
                            <div className="relative">
                              ...
                              <span className="absolute right-3 top-3.5 text-[10px] font-black text-slate-300 uppercase">kg</span>
                            </div>
                          )}
                        </div>
                        <div className="relative">
                           ...
                           <input type="text" className="w-full border-2 border-slate-50 p-3 pl-9 rounded-xl text-[10px] font-bold uppercase focus:border-blue-400 outline-none transition-all bg-slate-50" placeholder="Np. 8mm" value={newRecipe.grinding?.[c.id] || ''} onChange={e => setNewRecipe({...newRecipe, grinding: {...newRecipe.grinding, [c.id]: e.target.value}})} />
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="space-y-6">
                <textarea className="w-full border-2 border-slate-100 rounded-3xl p-6 h-40 text-sm focus:border-blue-500 outline-none shadow-sm bg-slate-50 focus:bg-white transition-all" value={newRecipe.description} onChange={e => setNewRecipe({...newRecipe, description: e.target.value})} placeholder="Procedura miazgi, wędzenia, parzenia..." />
                
                <div className="p-8 bg-slate-50 rounded-[2.5rem] border-2 border-slate-100">
                  <div className="flex justify-between items-center mb-6"><h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Przyprawy (g/kg)</h4> <button type="button" onClick={() => setNewRecipe({...newRecipe, spices: [...newRecipe.spices, {id:Date.now(), name:'', ratio:0}]})} className="bg-blue-600 text-white px-4 py-2 rounded-xl text-[10px] font-black shadow-lg hover:bg-blue-700 transition-all">+ DODAJ</button></div>
                  <div className="space-y-3 max-h-60 overflow-y-auto pr-2">
                    {newRecipe.spices.map((s, idx) => (
                      <div key={s.id} className="flex gap-2 bg-white p-3 rounded-2xl shadow-sm border border-slate-100 transition-all hover:border-blue-200">
                ...
                <div className="pt-6">
                    <button type="button" onClick={handleSave} disabled={!isValidated || !newRecipe.name} className={`w-full py-7 rounded-[2rem] font-black shadow-2xl transition-all uppercase tracking-widest text-sm ${(!isValidated || !newRecipe.name) ? 'bg-slate-100 text-slate-300 cursor-not-allowed border-2 border-dashed border-slate-200' : 'bg-blue-600 text-white hover:bg-blue-700 hover:scale-[1.01]'}`}>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div><span className="font-black text-slate-800 uppercase text-sm block mb-1 leading-none">{cls.label}</span>
                          <div className="flex gap-2">
                            <span className="text-[10px] font-black bg-blue-100 text-blue-700 px-2 py-0.5 rounded uppercase leading-none italic">Sugerowane: {(totalTarget * (recipe?.ratios?.[cls.id] || 0)).toFixed(2)} kg</span>
                        ...
                        {meatClasses.map(cls => (
                           recipe.grinding?.[cls.id] && (
                             <div key={cls.id} className="flex justify-between text-[11px] font-black uppercase text-blue-900">
                               <span className="opacity-50 leading-none italic">{cls.label}:</span>
